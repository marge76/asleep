...
$sql = "select distinct * from new com.ibm.db2j.GaianQuery('select runlog.guid,runlog.petname, day, distance, topspeed, city, loc_id, pettype,petsex,breed, petage, username from runlog,members where runlog.guid=members.guid and runlog.petname=members.petname and day >= ''".$dur."''";
...
if ($sort == 2) {
     $sql = $sql."') GQ order by topspeed desc";
} else {
	$sql = $sql."') GQ order by distance desc";
}
//echo $sql;
