...
def main(argv):
   ...
   global startdist
   if len(sys.argv) < 3 :
 	print 'rab_send.py -u <username> -p <petname> -d <diameter in cm> -s <start_distance>'
	sys.exit()
   try:
   ...
   for opt, arg in opts:
      if opt == '-h':
         print 'rab_send.py -u <username> -o <petname> -d <diameter in cm> -s <start_distance>'
         sys.exit()
      elif opt in ("-u", "--uname"):
         uname = arg
	...
	if GPIO.input(3) == False:
		...
		if end.day != start.day:
			...
			if speed < 4.0:
				...
				print "%s] Distance: %.4f cms Speed: %.2f m/s" % (datetime.datetime.now(), distance, speed)
				connection = pika.BlockingConnection(pika.ConnectionParameters('localhost'))
				channel = connection.channel()
				channel.queue_declare(queue='aatw_gaian')
